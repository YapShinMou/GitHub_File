	...
	for (int episode = 0; episode < EPISODES ; ++episode)
	{
		...
		while (!done) {
			...
			if (memory.size() >= BATCH_SIZE) {
				...
				auto state_tensor = DQNClass.batch_to_tensor(states); // [B, state_dim]
				auto next_state_tensor = DQNClass.batch_to_tensor(next_states); // [B, state_dim]
				auto action_tensor = torch::from_blob(actions.data(), {(long)actions.size()}, torch::kInt64).clone().to(DQNClass.device);
				auto reward_tensor = torch::from_blob(rewards.data(), {(long)rewards.size()}, torch::kFloat32).clone().to(DQNClass.device);
				auto done_tensor = torch::from_blob(dones.data(), {(long)dones.size()}, torch::kFloat32).clone().to(DQNClass.device);
				
				// Q values for taken actions
				auto q_values_all = DQNClass.policy_net->forward(state_tensor); // [B, A]
				auto q_values = q_values_all.gather(1, action_tensor.unsqueeze(1)).squeeze(1); // [B]
				
				// next Q values (target)
				auto next_q_all = DQNClass.target_net->forward(next_state_tensor);
				...
				DQNClass.optimizer->zero_grad();
				loss.backward();
				DQNClass.optimizer->step();
